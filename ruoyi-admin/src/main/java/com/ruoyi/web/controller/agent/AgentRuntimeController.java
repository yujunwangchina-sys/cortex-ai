...
import com.ruoyi.agent.runtime.approval.ApprovalManager;
import com.ruoyi.agent.runtime.context.SessionManager;
...
import com.ruoyi.agent.service.IAiAgentService;
...
public class AgentRuntimeController extends BaseController
{
    ...
    @Autowired
    private com.ruoyi.agent.mapper.AiAgentExecutionLogMapper executionLogMapper;

    @Autowired
    private com.ruoyi.agent.mapper.AiAgentApprovalGrantMapper approvalGrantMapper;
    ...
    public AjaxResult createSession(@RequestBody java.util.Map<String, String> params)
    {
        try
        {
            ...
            // 获取AgentId
            com.ruoyi.agent.domain.AiAgent agent = aiAgentService.selectAiAgentByCode(agentCode);
            if (agent == null)
            {
            ...
            // 创建会话
            com.ruoyi.agent.runtime.context.AgentSessionContext session = 
    ...
    {
        com.ruoyi.agent.domain.AiAgentSession detail = sessionManager.getSessionDetail(sessionId);
        if (detail != null)
        {
            // Load messages from Redis/DB instead of the deprecated JSON blob
            java.util.List<com.ruoyi.agent.runtime.model.ChatMessage> messages =
                    sessionManager.getSessionMessages(sessionId);
            com.alibaba.fastjson2.JSONArray msgArr = new com.alibaba.fastjson2.JSONArray();
            for (com.ruoyi.agent.runtime.model.ChatMessage msg : messages)
            {
    ...
    {
        startPage();
        com.ruoyi.agent.domain.AiAgentExecutionLog query = new com.ruoyi.agent.domain.AiAgentExecutionLog();
        query.setSessionId(sessionId);
        query.setEventType(eventType);
        query.setStatus(status);
        query.setBusinessSystem(businessSystem);
        query.setUserLoginName(userLoginName);
        java.util.List<com.ruoyi.agent.domain.AiAgentExecutionLog> list = executionLogMapper.selectAiAgentExecutionLogList(query);
        return getDataTable(list);
    }
    ...
    {
        startPage();
        com.ruoyi.agent.domain.AiAgentApprovalGrant query = new com.ruoyi.agent.domain.AiAgentApprovalGrant();
        query.setGrantStatus(grantStatus);
        query.setBusinessSystem(businessSystem);
        query.setSessionId(sessionId);
        java.util.List<com.ruoyi.agent.domain.AiAgentApprovalGrant> list = approvalGrantMapper.selectAiAgentApprovalGrantList(query);
        return getDataTable(list);
    }
