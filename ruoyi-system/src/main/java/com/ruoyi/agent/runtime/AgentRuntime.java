...
import com.ruoyi.agent.runtime.context.AgentSessionContext;
...
import com.ruoyi.agent.runtime.loop.ConversationLoop;
import com.ruoyi.agent.runtime.loop.MemoryManager;
...
import com.ruoyi.agent.runtime.prompt.PromptBuilder;
import com.ruoyi.agent.runtime.prompt.ToolSchemaBuilder;
...
public class AgentRuntime
{
    ...
    @Autowired
    private com.ruoyi.agent.runtime.file.FileContentParser fileParser;

    @Autowired
    private com.ruoyi.agent.runtime.file.ImageVisionFallback visionFallback;

    @Autowired
    private com.ruoyi.knowledge.rag.KnowledgeContextInjector knowledgeContextInjector;
    ...
    public AgentRunResult run(AgentRunRequest request, Consumer<SSEEvent> sseCallback) throws Exception
    {
        ...
        try
        {
            ...
            try {
                userNickName = com.ruoyi.common.utils.SecurityUtils.getLoginUser().getUser().getNickName();
            } catch (Exception e) {
            ...
            if (request.getFileIds() != null && !request.getFileIds().isEmpty()) {
                ...
                for (Long fid : request.getFileIds()) {
                    com.ruoyi.agent.domain.AiAgentFile af = fileService.selectAiAgentFileByFileId(fid);
                    if (af != null) {
                        // 关联文件到会话
