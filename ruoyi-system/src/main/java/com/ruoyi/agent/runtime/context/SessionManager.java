...
import com.ruoyi.agent.mapper.AiAgentSessionMapper;
...
public class SessionManager
{
    ...
    public AgentSessionContext getSession(String sessionId, int maxIterations)
    {
        ...
        List<com.ruoyi.agent.runtime.model.ChatMessage> loadedMessages = messageStore.loadMessages(sessionId);
        for (com.ruoyi.agent.runtime.model.ChatMessage msg : loadedMessages)
        {
            ctx.addMessage(msg);
        }
        ...
        if (session.getCompressedContext() != null && !session.getCompressedContext().isEmpty())
        {
            try
            {
                ...
                List<com.ruoyi.agent.runtime.model.ChatMessage> compressedMessages = new java.util.ArrayList<>();
                for (Object item : arr)
                {
                    compressedMessages.add(com.ruoyi.agent.runtime.model.ChatMessage.fromJson((com.alibaba.fastjson2.JSONObject) item));
                }
     ...
     */
    public void persistSession(AgentSessionContext ctx, Long modelId, com.ruoyi.supplier.domain.AiModel model, 
                               java.util.function.Consumer<com.ruoyi.agent.runtime.model.SSEEvent> sseCallback)
    {
        try
        {
            ...
            // 持久化压缩的工作上下文（如果存在）
            List<com.ruoyi.agent.runtime.model.ChatMessage> workingCtx = ctx.getWorkingContext();
            if (workingCtx != null && !workingCtx.isEmpty() && workingCtx.size() < ctx.getMessages().size())
            {
                ...
                for (com.ruoyi.agent.runtime.model.ChatMessage msg : workingCtx)
                {
                    arr.add(msg.toJson());
                }
     ...
     * 通过 SSE 实时推送标题给前端
     */
    private void generateAndUpdateTitle(AgentSessionContext ctx, Long modelId, com.ruoyi.supplier.domain.AiModel model,
                                       java.util.function.Consumer<com.ruoyi.agent.runtime.model.SSEEvent> sseCallback)
    {
        try
        {
            ...
            session.setUpdateTime(new Date());
                ...
                if (sseCallback != null)
                {
                    sseCallback.accept(com.ruoyi.agent.runtime.model.SSEEvent.titleGenerated(
     ...
     * 使用用户选择的模型和模型配置的参数
     */
    private String generateTitle(String userMessage, Long modelId, com.ruoyi.supplier.domain.AiModel modelConfig)
    {
        try
        {
            // 1. 获取模型配置
            com.ruoyi.supplier.domain.AiModel model = modelConfig;
            ...
            {
                // 获取默认的chat模型
                com.ruoyi.supplier.domain.AiModel query = new com.ruoyi.supplier.domain.AiModel();
                query.setModelType("chat");
                query.setStatus("0");
                java.util.List<com.ruoyi.supplier.domain.AiModel> models = modelMapper.selectAiModelList(query);
            ...
            // 2. 获取供应商配置
            com.ruoyi.supplier.domain.AiSupplier supplier = supplierMapper.selectAiSupplierBySupplierId(model.getSupplierId());
            if (supplier == null)
            {
            ...
            // 3. 构建LLM请求
            com.ruoyi.agent.runtime.llm.LlmRequest request = new com.ruoyi.agent.runtime.llm.LlmRequest();
            request.setBaseUrl(supplier.getApiBaseUrl());
            request.setApiKey(supplier.getApiKey());
            request.setModel(model.getModelCode());
            
            // 4. 构建消息
            java.util.List<com.ruoyi.agent.runtime.model.ChatMessage> messages = new java.util.ArrayList<>();
            messages.add(com.ruoyi.agent.runtime.model.ChatMessage.system(
                    "你是一个会话标题生成助手。根据用户的第一条消息，识别用户的意图，生成一个简洁的标题，不超过20个字，直接输出标题文本，不要有任何解释或标点符号。"
            ));
            messages.add(com.ruoyi.agent.runtime.model.ChatMessage.user(
                    "用户消息：" + userMessage
            ));
            ...
            // 6. 调用LLM
            com.ruoyi.agent.runtime.llm.LlmResponse response = llmClient.chatCompletion(request);
     ...
     */
    public java.util.List<com.ruoyi.agent.runtime.model.ChatMessage> getSessionMessages(String sessionId)
    {
