...
import com.cortex.plugin.service.IBuiltinPluginScanService;
import com.cortex.plugin.service.IMcpPackageScanService;
...
public class McpPackageScanServiceImpl implements IMcpPackageScanService {
    ...
    public McpPackageScanResult scanAllMcpPackages() {
        ...
        // 扫描内置插件
        List<com.cortex.plugin.domain.vo.BuiltinPluginInfo> builtinPlugins = new ArrayList<>();
        ...
        int configuredCount = (int) pythonPackages.stream().filter(McpPackageInfo::getEnabled).count()
                ...
                + (int) builtinPlugins.stream().filter(com.cortex.plugin.domain.vo.BuiltinPluginInfo::getEnabled).count();
